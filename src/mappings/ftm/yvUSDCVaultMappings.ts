...
} from '../../../generated/ftmRvUSDCVault/Vault';
...
export function handleAddStrategyV2(call: AddStrategyV2Call): void {
  if (vaultLibrary.isVault(call.to) && vaultLibrary.isVault(call.from)) {
    log.warning(
      'ftmRvUSDCVault_AddStrategyV2(...) - TX {} - Call to {} and call from {} are vaults (minimal proxy). Not processing addStrategy tx.',
      [
  ...
  if (
    isEventBlockNumberLt(
      'ftmRvUSDCVault_AddStrategyV2Call',
      call.block,
      FTM_YV_USDC_VAULT_END_BLOCK_CUSTOM
    )
  ) {
...
export function handleAddStrategy(call: AddStrategyV1Call): void {
  if (vaultLibrary.isVault(call.to) && vaultLibrary.isVault(call.from)) {
    log.warning(
      'ftmRvUSDCVault_AddStrategy(...) - TX {} - Call to {} and call from {} are vaults (minimal proxy). Not processing addStrategy tx.',
      [
        call.transaction.hash.toHexString(),
        call.to.toHexString(),
        call.from.toHexString(),
  ...
  if (
    isEventBlockNumberLt(
      'ftmRvUSDCVault_AddStrategyCall',
      call.block,
      FTM_YV_USDC_VAULT_END_BLOCK_CUSTOM
    )
  ) {
...
export function handleStrategyReported_v0_3_0_v0_3_1(
  ...
  if (
    isEventBlockNumberLt(
      'ftmRvUSDCVault_StrategyReportedEvent',
      event.block,
      FTM_YV_USDC_VAULT_END_BLOCK_CUSTOM
    )
  ) {
...
export function handleStrategyMigrated(event: StrategyMigrated): void {
  if (
    ...
    let ethTransaction = getOrCreateTransactionFromEvent(
      event,
      'ftmRvUSDCVault_StrategyMigratedEvent'
    );
...
export function handleDeposit(call: DepositCall): void {
  ...
  if (vaultLibrary.isVault(call.to) && vaultLibrary.isVault(call.from)) {
    log.warning(
      'ftmRvUSDCVault_Deposit () - TX {} - Call to {} and call from {} are vaults (minimal proxy). Not processing deposit tx.',
      [
        call.transaction.hash.toHexString(),
        call.to.toHexString(),
        call.from.toHexString(),
...
export function handleDepositWithAmount(call: Deposit1Call): void {
  ...
  if (vaultLibrary.isVault(call.to) && vaultLibrary.isVault(call.from)) {
    log.warning(
      'ftmRvUSDCVault_Deposit (amount) - TX {} - Call to {} and call from {} are vaults (minimal proxy). Not processing deposit tx.',
      [
        call.transaction.hash.toHexString(),
        call.to.toHexString(),
        call.from.toHexString(),
  ...
  if (
    isEventBlockNumberLt(
      'ftmRvUSDCVault_vault.deposit(uint)',
      call.block,
      FTM_YV_USDC_VAULT_END_BLOCK_CUSTOM
    )
  ) {
...
export function handleDepositWithAmountAndRecipient(call: Deposit2Call): void {
  ...
  if (vaultLibrary.isVault(call.to) && vaultLibrary.isVault(call.from)) {
    log.warning(
      'ftmRvUSDCVault_Deposit (amount,recipient) - TX {} - Call to {} and call from {} are vaults (minimal proxy). Not processing deposit tx.',
      [
        call.transaction.hash.toHexString(),
        call.to.toHexString(),
        call.from.toHexString(),
  ...
  if (
    isEventBlockNumberLt(
      'ftmRvUSDCVault_vault.deposit(uint,address)',
      call.block,
      FTM_YV_USDC_VAULT_END_BLOCK_CUSTOM
    )
  ) {
...
export function handleWithdraw(call: WithdrawCall): void {
  ...
  if (vaultLibrary.isVault(call.to) && vaultLibrary.isVault(call.from)) {
    log.warning(
      'ftmRvUSDCVault_Withdraw (shares) - TX {} - Call to {} and call from {} are vaults (minimal proxy). Not processing withdraw tx.',
      [
        call.transaction.hash.toHexString(),
        call.to.toHexString(),
        call.from.toHexString(),
...
export function handleWithdrawWithShares(call: Withdraw1Call): void {
  ...
  if (
    isEventBlockNumberLt(
      'ftmRvUSDCVault_vault.withdraw(uint256)',
      call.block,
      FTM_YV_USDC_VAULT_END_BLOCK_CUSTOM
    )
  ) {
...
export function handleWithdrawWithSharesAndRecipient(
  ...
  if (vaultLibrary.isVault(call.to) && vaultLibrary.isVault(call.from)) {
    log.warning(
      'ftmRvUSDCVault_Withdraw (shares,recipient) - TX {} - Call to {} and call from {} are vaults (minimal proxy). Not processing withdraw tx.',
      [
        call.transaction.hash.toHexString(),
        call.to.toHexString(),
        call.from.toHexString(),
  ...
  if (
    isEventBlockNumberLt(
      'ftmRvUSDCVault_vault.withdraw(uint256,address)',
      call.block,
      FTM_YV_USDC_VAULT_END_BLOCK_CUSTOM
    )
  ) {
...
export function handleWithdrawWithSharesAndRecipientAndMaxLoss(
  ...
  if (vaultLibrary.isVault(call.to) && vaultLibrary.isVault(call.from)) {
    log.warning(
      'ftmRvUSDCVault_Withdraw (shares,recipient,maxLoss) - TX {} - Call to {} and call from {} are vaults (minimal proxy). Not processing withdraw tx.',
      [
        call.transaction.hash.toHexString(),
        call.to.toHexString(),
        call.from.toHexString(),
  ...
  if (
    isEventBlockNumberLt(
      'ftmRvUSDCVault_vault.withdraw(uint256,address,uint256)',
      call.block,
      FTM_YV_USDC_VAULT_END_BLOCK_CUSTOM
    )
  ) {
...
export function handleTransfer(event: TransferEvent): void {
  ...
  if (
    isEventBlockNumberLt(
      'ftmRvUSDCVault_vault.transfer(address,uint256)',
      event.block,
      FTM_YV_USDC_VAULT_END_BLOCK_CUSTOM
    )
  ) {
...
export function handleUpdatePerformanceFee(
  ...
  if (
    isEventBlockNumberLt(
      'ftmRvUSDCVault_UpdatePerformanceFee',
      event.block,
      FTM_YV_USDC_VAULT_END_BLOCK_CUSTOM
    )
  ) {
...
export function handleUpdateManagementFee(
  ...
  if (
    isEventBlockNumberLt(
      'ftmRvUSDCVault_UpdateManagementFee',
      event.block,
      FTM_YV_USDC_VAULT_END_BLOCK_CUSTOM
    )
  ) {
...
export function handleStrategyAddedToQueue(
  ...
  if (
    isEventBlockNumberLt(
      'ftmRvUSDCVault_StrategyAddedToQueue',
      event.block,
      FTM_YV_USDC_VAULT_END_BLOCK_CUSTOM
    )
  ) {
...
export function handleStrategyRemovedFromQueue(
  ...
  if (
    isEventBlockNumberLt(
      'ftmRvUSDCVault_StrategyRemovedFromQueue',
      event.block,
      FTM_YV_USDC_VAULT_END_BLOCK_CUSTOM
    )
  ) {
...
export function handleUpdateRewards(event: UpdateRewardsEvent): void {
  if (
    isEventBlockNumberLt(
      'ftmRvUSDCVault_UpdateRewardsEvent',
      event.block,
      FTM_YV_USDC_VAULT_END_BLOCK_CUSTOM
    )
  ) {
