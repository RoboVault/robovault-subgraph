...
} from '../../../generated/RoboVaultV3USDC/Vault';
import { Strategy, Transaction, Vault } from '../../../generated/schema';
import { isEventBlockNumberLt, printCallInfo } from '../../utils/commons';
import {
  BIGINT_ZERO,
  ZERO_ADDRESS,
  // TODO: droidmuncher: Need to figure out the significance of this block number
  FTM_ROBO_VAULT_V3_USDC_END_BLOCK_CUSTOM,
...
function createRoboVaultV3USDCIfNeeded(
  ...
  return vaultLibrary.createCustomVaultIfNeeded(
    ...
    // TODO: droidmuncher: This should be the registry address, this will probably create a registry entity at 0x0 address which could break things
    changetype<Address>(Address.fromHexString(ZERO_ADDRESS)),
    EXPERIMENTAL,
    API_VERSION_0_4_2,
    transaction,
    DON_T_CREATE_VAULT_TEMPLATE
...
export function handleStrategyAddedV2(event: StrategyAddedV2Event): void {
  if (
    isEventBlockNumberLt(
      'RoboVaultV3USDC_AddStrategyV2Event',
      event.block,
      FTM_ROBO_VAULT_V3_USDC_END_BLOCK_CUSTOM
    )
  ) {
...
export function handleStrategyReported_v0_3_0_v0_3_1(
  ...
  if (
    isEventBlockNumberLt(
      'RoboVaultV3USDC_StrategyReportedEvent',
      event.block,
      FTM_ROBO_VAULT_V3_USDC_END_BLOCK_CUSTOM
    )
  ) {
...
export function handleStrategyMigrated(event: StrategyMigrated): void {
  if (
    ...
    let ethTransaction = getOrCreateTransactionFromEvent(
      event,
      'RoboVaultV3USDC_StrategyMigratedEvent'
    );
...
export function handleDeposit(call: DepositCall): void {
  ...
  if (vaultLibrary.isVault(call.to) && vaultLibrary.isVault(call.from)) {
    log.warning(
      'RoboVaultV3USDC_Deposit () - TX {} - Call to {} and call from {} are vaults (minimal proxy). Not processing deposit tx.',
      [
  ...
  if (
    ...
    createRoboVaultV3USDCIfNeeded(call.to, transaction);
...
export function handleDepositWithAmount(call: Deposit1Call): void {
  ...
  if (vaultLibrary.isVault(call.to) && vaultLibrary.isVault(call.from)) {
    log.warning(
      'RoboVaultV3USDC_Deposit (amount) - TX {} - Call to {} and call from {} are vaults (minimal proxy). Not processing deposit tx.',
      [
        call.transaction.hash.toHexString(),
        call.to.toHexString(),
        call.from.toHexString(),
  ...
  if (
    isEventBlockNumberLt(
      'RoboVaultV3USDC_vault.deposit(uint)',
      call.block,
      FTM_ROBO_VAULT_V3_USDC_END_BLOCK_CUSTOM
    )
  ) {
...
export function handleDepositWithAmountAndRecipient(call: Deposit2Call): void {
  ...
  if (vaultLibrary.isVault(call.to) && vaultLibrary.isVault(call.from)) {
    log.warning(
      'RoboVaultV3USDC_Deposit (amount,recipient) - TX {} - Call to {} and call from {} are vaults (minimal proxy). Not processing deposit tx.',
      [
        call.transaction.hash.toHexString(),
        call.to.toHexString(),
        call.from.toHexString(),
  ...
  if (
    isEventBlockNumberLt(
      'RoboVaultV3USDC_vault.deposit(uint,address)',
      call.block,
      FTM_ROBO_VAULT_V3_USDC_END_BLOCK_CUSTOM
    )
  ) {
...
export function handleWithdraw(call: WithdrawCall): void {
  ...
  if (vaultLibrary.isVault(call.to) && vaultLibrary.isVault(call.from)) {
    log.warning(
      'RoboVaultV3USDC_Withdraw (shares) - TX {} - Call to {} and call from {} are vaults (minimal proxy). Not processing withdraw tx.',
      [
        call.transaction.hash.toHexString(),
        call.to.toHexString(),
        call.from.toHexString(),
  ...
  if (
    isEventBlockNumberLt(
      'RoboVaultV3USDC_vault.withdraw()',
      call.block,
      FTM_ROBO_VAULT_V3_USDC_END_BLOCK_CUSTOM
    )
  ) {
...
export function handleWithdrawWithShares(call: Withdraw1Call): void {
  ...
  if (
    isEventBlockNumberLt(
      'RoboVaultV3USDC_vault.withdraw(uint256)',
      call.block,
      FTM_ROBO_VAULT_V3_USDC_END_BLOCK_CUSTOM
    )
  ) {
...
export function handleWithdrawWithSharesAndRecipient(
  ...
  if (vaultLibrary.isVault(call.to) && vaultLibrary.isVault(call.from)) {
    log.warning(
      'RoboVaultV3USDC_Withdraw (shares,recipient) - TX {} - Call to {} and call from {} are vaults (minimal proxy). Not processing withdraw tx.',
      [
        call.transaction.hash.toHexString(),
        call.to.toHexString(),
        call.from.toHexString(),
  ...
  if (
    isEventBlockNumberLt(
      'RoboVaultV3USDC_vault.withdraw(uint256,address)',
      call.block,
      FTM_ROBO_VAULT_V3_USDC_END_BLOCK_CUSTOM
    )
  ) {
...
export function handleWithdrawWithSharesAndRecipientAndMaxLoss(
  ...
  if (vaultLibrary.isVault(call.to) && vaultLibrary.isVault(call.from)) {
    log.warning(
      'RoboVaultV3USDC_Withdraw (shares,recipient,maxLoss) - TX {} - Call to {} and call from {} are vaults (minimal proxy). Not processing withdraw tx.',
      [
        call.transaction.hash.toHexString(),
        call.to.toHexString(),
        call.from.toHexString(),
  ...
  if (
    isEventBlockNumberLt(
      'RoboVaultV3USDC_vault.withdraw(uint256,address,uint256)',
      call.block,
      FTM_ROBO_VAULT_V3_USDC_END_BLOCK_CUSTOM
    )
  ) {
...
export function handleTransfer(event: TransferEvent): void {
  ...
  if (
    isEventBlockNumberLt(
      'RoboVaultV3USDC_vault.transfer(address,uint256)',
      event.block,
      FTM_ROBO_VAULT_V3_USDC_END_BLOCK_CUSTOM
    )
  ) {
    if (
      ...
      createRoboVaultV3USDCIfNeeded(event.address, transaction);
...
export function handleUpdatePerformanceFee(
  ...
  if (
    isEventBlockNumberLt(
      'RoboVaultV3USDC_UpdatePerformanceFee',
      event.block,
      FTM_ROBO_VAULT_V3_USDC_END_BLOCK_CUSTOM
    )
  ) {
    ...
    createRoboVaultV3USDCIfNeeded(event.address, ethTransaction);
...
export function handleUpdateManagementFee(
  ...
  if (
    isEventBlockNumberLt(
      'RoboVaultV3USDC_UpdateManagementFee',
      event.block,
      FTM_ROBO_VAULT_V3_USDC_END_BLOCK_CUSTOM
    )
  ) {
...
export function handleStrategyAddedToQueue(
  ...
  if (
    isEventBlockNumberLt(
      'RoboVaultV3USDC_StrategyAddedToQueue',
      event.block,
      FTM_ROBO_VAULT_V3_USDC_END_BLOCK_CUSTOM
    )
  ) {
...
export function handleStrategyRemovedFromQueue(
  ...
  if (
    isEventBlockNumberLt(
      'RoboVaultV3USDC_StrategyRemovedFromQueue',
      event.block,
      FTM_ROBO_VAULT_V3_USDC_END_BLOCK_CUSTOM
    )
  ) {
...
export function handleUpdateRewards(event: UpdateRewardsEvent): void {
  if (
    isEventBlockNumberLt(
      'RoboVaultV3USDC_UpdateRewardsEvent',
      event.block,
      FTM_ROBO_VAULT_V3_USDC_END_BLOCK_CUSTOM
    )
  ) {
