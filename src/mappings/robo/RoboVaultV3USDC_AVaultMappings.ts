...
export function handleStrategyAddedV2(event: StrategyAddedV2Event): void {
  let transaction = getOrCreateTransactionFromEvent(
    event,
    'RoboVaultV3USDC_A_AddStrategyV2Event'
  );
...
export function handleStrategyReported_v0_3_0_v0_3_1(
  ...
  let ethTransaction = getOrCreateTransactionFromEvent(
    event,
    'RoboVaultV3USDC_A_StrategyReportedEvent'
  );
...
export function handleStrategyMigrated(event: StrategyMigrated): void {
  ...
  let ethTransaction = getOrCreateTransactionFromEvent(
    event,
    'RoboVaultV3USDC_A_StrategyMigratedEvent'
  );
  ...
  if (oldStrategy !== null) {
    ...
    if (Strategy.load(newStrategyAddress.toHexString()) !== null) {
      log.warning(
        '[RoboVaultV3USDC_A Strategy Migrated] Migrating to strategy {} but it has already been created',
...
export function handleDeposit(call: DepositCall): void {
  ...
  if (vaultLibrary.isVault(call.to) && vaultLibrary.isVault(call.from)) {
    log.warning(
      'RoboVaultV3USDC_A_Deposit () - TX {} - Call to {} and call from {} are vaults (minimal proxy). Not processing deposit tx.',
      [
  ...
  let transaction = getOrCreateTransactionFromCall(
    call,
    'RoboVaultV3USDC_A_vault.deposit()'
  );
...
export function handleDepositWithAmount(call: Deposit1Call): void {
  ...
  if (vaultLibrary.isVault(call.to) && vaultLibrary.isVault(call.from)) {
    log.warning(
      'RoboVaultV3USDC_A_Deposit (amount) - TX {} - Call to {} and call from {} are vaults (minimal proxy). Not processing deposit tx.',
      [
        call.transaction.hash.toHexString(),
        call.to.toHexString(),
        call.from.toHexString(),
  ...
  let transaction = getOrCreateTransactionFromCall(
    call,
    'RoboVaultV3USDC_A_vault.deposit(uint)'
  );
  createVaultIfNeeded(call.to, transaction);
  vaultLibrary.deposit(
...
export function handleDepositWithAmountAndRecipient(call: Deposit2Call): void {
  ...
  if (vaultLibrary.isVault(call.to) && vaultLibrary.isVault(call.from)) {
    log.warning(
      'RoboVaultV3USDC_A_Deposit (amount,recipient) - TX {} - Call to {} and call from {} are vaults (minimal proxy). Not processing deposit tx.',
      [
        call.transaction.hash.toHexString(),
        call.to.toHexString(),
        call.from.toHexString(),
  ...
  let transaction = getOrCreateTransactionFromCall(
    call,
    'RoboVaultV3USDC_A_vault.deposit(uint,address)'
  );
  createVaultIfNeeded(call.to, transaction);
  log.info(
...
export function handleWithdraw(call: WithdrawCall): void {
  ...
  if (vaultLibrary.isVault(call.to) && vaultLibrary.isVault(call.from)) {
    log.warning(
      'RoboVaultV3USDC_A_Withdraw (shares) - TX {} - Call to {} and call from {} are vaults (minimal proxy). Not processing withdraw tx.',
      [
        call.transaction.hash.toHexString(),
        call.to.toHexString(),
        call.from.toHexString(),
  ...
  let transaction = getOrCreateTransactionFromCall(
    call,
    'RoboVaultV3USDC_A_vault.withdraw()'
  );
...
export function handleWithdrawWithShares(call: Withdraw1Call): void {
  ...
  let transaction = getOrCreateTransactionFromCall(
    call,
    'RoboVaultV3USDC_A_vault.withdraw(uint256)'
  );
...
export function handleWithdrawWithSharesAndRecipient(
  ...
  if (vaultLibrary.isVault(call.to) && vaultLibrary.isVault(call.from)) {
    log.warning(
      'RoboVaultV3USDC_A_Withdraw (shares,recipient) - TX {} - Call to {} and call from {} are vaults (minimal proxy). Not processing withdraw tx.',
      [
        call.transaction.hash.toHexString(),
        call.to.toHexString(),
        call.from.toHexString(),
  ...
  let transaction = getOrCreateTransactionFromCall(
    call,
    'RoboVaultV3USDC_A_vault.withdraw(uint256,address)'
  );
  log.info(
...
export function handleWithdrawWithSharesAndRecipientAndMaxLoss(
  ...
  if (vaultLibrary.isVault(call.to) && vaultLibrary.isVault(call.from)) {
    log.warning(
      'RoboVaultV3USDC_A_Withdraw (shares,recipient,maxLoss) - TX {} - Call to {} and call from {} are vaults (minimal proxy). Not processing withdraw tx.',
      [
        call.transaction.hash.toHexString(),
        call.to.toHexString(),
        call.from.toHexString(),
  ...
  let transaction = getOrCreateTransactionFromCall(
    call,
    'RoboVaultV3USDC_A_vault.withdraw(uint256,address,uint256)'
  );
  log.info(
...
export function handleTransfer(event: TransferEvent): void {
  ...
  if (
    ...
    let transaction = getOrCreateTransactionFromEvent(
      event,
      'RoboVaultV3USDC_A_vault.transfer(address,uint256)'
    );
...
export function handleUpdatePerformanceFee(
  ...
  let ethTransaction = getOrCreateTransactionFromEvent(
    event,
    'RoboVaultV3USDC_A_UpdatePerformanceFee'
  );
...
export function handleUpdateManagementFee(
  ...
  let ethTransaction = getOrCreateTransactionFromEvent(
    event,
    'RoboVaultV3USDC_A_UpdateManagementFee'
  );
  createVaultIfNeeded(event.address, ethTransaction);
...
export function handleStrategyAddedToQueue(
  ...
  let ethTransaction = getOrCreateTransactionFromEvent(
    event,
    'RoboVaultV3USDC_A_StrategyAddedToQueue'
  );
...
export function handleStrategyRemovedFromQueue(
  ...
  let ethTransaction = getOrCreateTransactionFromEvent(
    event,
    'RoboVaultV3USDC_A_StrategyRemovedFromQueue'
  );
  vaultLibrary.strategyRemovedFromQueue(
    event.params.strategy,
    ethTransaction,
...
export function handleUpdateRewards(event: UpdateRewardsEvent): void {
  let ethTransaction = getOrCreateTransactionFromEvent(
    event,
    'RoboVaultV3USDC_A_UpdateRewardsEvent'
  );
